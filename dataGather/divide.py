products = ['Animal','Textiles and Clothing','Wood','Minerals','Food Products','Chemicals','Plastic or Rubber','Fuels','Mach and Elec']
...
def main():
    ...
    while i<len(years):
            csv = open('relationaltradepv.csv', 'r')
            lines = csv.readlines()
            filename = './traderel/trade'+years[i]+'.csv'
            print(filename)
            f=open(filename, 'w')
            if (i==len(years)):
            ...
            for line in lines:
                ...
                if "," in row[1]:
                    row[1] ="\""+row[1]+"\""
                if ((float(row[3+i])!=0) and (row[2] in products)):
                    if (i==len(years)):
